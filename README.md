...
- **KD 计算**：由模组/插件计算后上传，机器人**不参与计算**，仅存储展示
- **QQ 查询命令**：
...
**群聊中需 @机器人 才会回复；未 @机器人 时不会响应任何消息。** 亦可与机器人单聊发送。
...
KD 由模组（插件）计算后通过 `/api/plugin/*` 上传，机器人只负责存储与展示，不参与 KD 计算。